...
# Global toggle: disable dotted focus rings (set to True to remove focus outlines)
FOCUS_RING_DISABLED = True
...
def apply_focus_ring_setting(root) -> None:
    """Apply a best-effort global setting to remove dotted focus outlines.

    This attempts several approaches because different Tk themes and widgets
    (classic tk vs ttk) expose focus visuals differently. It is intentionally
    defensive and non-fatal if a particular approach isn't available.
    """
    if not globals().get("FOCUS_RING_DISABLED", False):
        return
    # We keep a small, focused implementation: create a 'focus sink' and only
    # redirect mouse-initiated focus there. This preserves keyboard navigation
    # while avoiding multiple redundant style tweaks that may break themes.
    try:
        try:
            focus_sink = tk.Frame(root, width=0, height=0, bd=0, highlightthickness=0)
            focus_sink.place(x=-100, y=-100)
            root._focus_sink = focus_sink
        except Exception:
            root._focus_sink = None

        root._mouse_focus_pending = False

        def _on_button_press(e):
            try:
                root._mouse_focus_pending = True
            except Exception:
                pass

        def _on_focus_in(e):
            try:
                if getattr(root, '_mouse_focus_pending', False):
                    root._mouse_focus_pending = False
                    if getattr(root, '_focus_sink', None):
                        try:
                            root.after_idle(lambda: root._focus_sink.focus_set())
                        except Exception:
                            try:
                                root._focus_sink.focus_set()
                            except Exception:
                                pass
            except Exception:
                pass
        ...
        try:
            root.bind_all('<ButtonPress>', _on_button_press, add='+')
...
def _read_int_key_from_text(content: str, key: str):
    """Return int value for "key": <int> or None if not present/parsable."""
...
def prompt_save_version_mismatch_and_choose(path, modal=True):
    ...
    def _validate_and_apply_new_path(new_path):
        ...
        try:
            ...
            m, r, d, t, s, day, night, tp = get_file_info(new_content)
...
def modify_rules(file_path, difficulty, truck_avail, truck_price, internal_avail, internal_amount=None):
    """Rules functionality removed — stub preserved for compatibility."""
    # Intentionally left blank; rules UI removed per user request.
    return
...
# ---------- BEGIN: Final Rules tab (full, robust, ready-to-paste) ----------
import os
import re
import json
import shutil
# tk/ttk/messagebox assumed imported in your main file already
...
def launch_gui():
    ...
    # Apply the global focus-ring suppression helper (best-effort)
    try:
        apply_focus_ring_setting(root)
    except Exception:
        pass

    # Update variables with settings from config
