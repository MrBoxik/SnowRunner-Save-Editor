...
class VirtualObjectivesFast:
    def __init__(self, parent, save_path_var):
        ...
        try:
            ...
            pass
    def _tk_var_get(self, var, default=None):
        """
    ...
    def _watch_save_var(self):
        try:
            current = self._tk_var_get(self.save_var)
    ...
    def load_data_thread(self):
        ...
        def worker():
            ...
            sp = self._tk_var_get(self.save_var)
            pre = set()
...
def launch_gui():
    ...
    def update_money_only():
        ...
        if not v.isdigit():
            return messagebox.showerror("Invalid", "Money must be a non-negative integer.")
        if not _write_json_key_to_file(path, "money", int(v)):
        ...
        try:
            if "sync_all_rules" in globals():
                ...
                money_var.set(str(int(v)))
        except Exception as e:
            print("Warning: failed to refresh GUI after money update:", e)
        messagebox.showinfo("Success", f"Money updated to {v}.")
