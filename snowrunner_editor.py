...
FACTOR_RULE_DEFINITIONS = [
    ("Region Repair Price", "regionRepaireMoneyFactor", {"default": 1, "2 times outside home region": 2, "3 times outside home region": 3, "4 times outside home region": 4}),
    ("Recovery Price", "recoveryPriceFactor", {"free": 0, "paid": 1, "2 times": 2, "4 times": 4, "6 times": 6}),
    ("Automatic Cargo Loading Price", "loadingPriceFactor", {"free": 0, "paid": 1, "2 times": 2, "4 times": 4, "6 times": 6}),
    ("Region Travelling Price", "regionTravellingPriceFactor", {"free": 0, "paid": 1, "2 times": 2, "4 times": 4, "6 times": 6}),
...
def launch_gui():
    ...
    "Disco++ [WILL KILL YOUR EYES]": (100000.0, 100000.0),
}
    ...
    # Money tab
    ttk.Label(tab_money, text="Current Money:").pack()
...
# plugin_loaders = []  # Removed duplicate to preserve earlier plugin loader
    ...
    damage_options = {'default': 1, 'no damage': 0, '2 times': 2, '3 times': 3, '5 times': 5}
    ...
    trailer_map = {"free": 0, "normal price": 1, "2 times": 2, "4 times": 4, "6 times": 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    addon_map = {"default": 1, "free": 0, "2 times": 2, "4 times": 4, "6 times": 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    fuel_map = {"free": 0, "normal price": 1, "2 times": 2, "4 times": 4, "6 times": 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ttk.Label(subframe, text="Garage Refuel:").pack(anchor="w", pady=2)
    ...
    repair_map = {"free": -1, "normal price": 0, "2 times": 2, "4 times": 4, "6 times": 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    repair_cost_map = {'free': 0, 'paid': 1, '2times': 2, '4times': 4, '6times': 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    repair_req_map = {'default': 1, '2 times less': 0.5, '2times': 2, '4 times': 4, '6 times': 6}
    subframe = ttk.Frame(rules_grid_frame, relief="groove", padding=5)
    subframe.grid(row=rule_index // rule_col_count, column=rule_index % rule_col_count, padx=5, pady=5, sticky="n")
    ...
    ttk.Checkbutton(tab_time, text="Able To Skip Time", variable=skip_time_var).pack(pady=10)
    ttk.Label(tab_time, text="👉 To use the sliders below, please select 'Custom' in Time Presets.", foreground="red", font=("TkDefaultFont", 9, "bold")).pack(pady=(5, 10))


    def update_slider_labels():
        day_label.config(text=f"{custom_day_var.get():.1f}")
        night_label.config(text=f"{custom_night_var.get():.1f}")

    custom_day_var.trace_add("write", lambda *args: update_slider_labels())
    custom_night_var.trace_add("write", lambda *args: update_slider_labels())

    frame_day = ttk.Frame(tab_time)
    frame_day.pack()
    ttk.Label(frame_day, text="Custom Day Time:").pack(side="left")
    ttk.Scale(frame_day, from_=-5.0, to=5.0, variable=custom_day_var, orient="horizontal", length=250).pack(side="left", padx=5)
    day_label = ttk.Label(frame_day, text="1.0")
    day_label.pack(side="left")
    custom_day_var.set(round(day, 2))
    day_label.config(text=f"{round(day, 2):.1f}")

    frame_night = ttk.Frame(tab_time)
    frame_night.pack()
    ttk.Label(frame_night, text="Custom Night Time:").pack(side="left")
    ttk.Scale(frame_night, from_=-5.0, to=5.0, variable=custom_night_var, orient="horizontal", length=250).pack(side="left", padx=5)
    night_label = ttk.Label(frame_night, text="1.0")
    night_label.pack(side="left")
    custom_night_var.set(round(night, 2))
    night_label.config(text=f"{round(night, 2):.1f}")

    ttk.Label(tab_time, text="""ℹ️ Time Modifier Guide:
1.0 = normal speed
2.0 = double speed
0.0 = time stops
-1.0 = time reverses
-2.0 = reverse double speed
...
time will freeze at the transition (day to night or night to day).""", wraplength=400, justify="left", foreground="darkblue").pack(pady=(10, 20))
    ...
    ttk.Button(tab_time, text="Update Time", command=update_time_btn).pack(pady=20)
