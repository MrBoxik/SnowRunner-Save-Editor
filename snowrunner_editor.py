...
def create_contest_tab(tab, save_path_var):
    ...
    seasons = [
        "Season 1: Kola", "Season 2: Yukon", "Season 3: Wisconsin",
        "Season 4: Amur", "Season 5: Don", "Season 6: Maine",
        "Season 7: Tennessee", "Season 8: Glades", "Season 9: Ontario",
        "Season 10: British Columbia", "Season 11: Scandinavia", "Season 12: North Caroline",
        "Season 13: Almaty", "Season 14: Austria", "Season 15: Quebec"
    ]
    ...
    for i, name in enumerate(seasons, 1):
        var = tk.IntVar()
        ttk.Checkbutton(season_frame, text=name, variable=var).grid(row=i // 2, column=i % 2, sticky="w")
        season_vars.append((i, var))

    ttk.Label(tab, text="Other Season Code (e.g. 16, 17):").pack()
...
def create_upgrades_tab(tab, save_path_var):
    ...
    ttk.Label(tab, text="If a new season is added, you may need to mark or collect one upgrade in that region first.",
              foreground="red").pack()
...
def launch_gui():
    ...
    "Long Day": (0.1, 1.0),
    "Long Night": (1.0, 0.1),
    "Long Day and Long Night": (0.1, 0.1),
    "Time Stops": (0.0, 0.0),
